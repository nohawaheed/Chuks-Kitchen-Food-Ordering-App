...
function DeliveryDetails({ onClick }) {
  return (
    <div className="flex flex-col max-w-216 lg:w-216 m-auto mt-15 gap-4.5 px-1.75 md:px-4.5 py-9 rounded-lg bg-white">
      ...
      <div className="flex justify-between gap-4.75 text-area">
        <p>
          Home: 123 Main Street, Victoria Island, Lagos <br /> Apt 4B, Opposite
          Mega Plaza
        </p>
        <button className="text-primary-blue text-small-body min-w-23.75">
          Change Address
        </button>
      </div>
      <div className="flex flex-col gap-4.75">
        ...
        <input
          className="px-2.5 pt-2.5 pb-6.5 rounded-lg ring-[1px] ring-inset ring-stroke placeholder:text-black"
          type="text"
          placeholder="ASAP(30-25)"
        />
      </div>
      <div className="flex flex-col gap-4.75">
        <h3 className="text-medium-title font-semibold">
      ...
      <Button onClick={onClick} title="Proceed to Payment" />
    </div>
  );
}
